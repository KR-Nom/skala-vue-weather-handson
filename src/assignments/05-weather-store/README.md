...
- 여러 View에서 유지해야 하는 온도 단위는 각 컴포넌트의 `ref`보다 Store에서 관리하는 것이 알맞다.
- Store 상태를 template에서 사용할 때와 Store 내부 함수에서 사용할 때 `.value` 사용 방법이 다르다.
- 섭씨·화씨 전환은 단위 기호만 바꾸는 것이 아니라 실제 숫자도 변환해야 한다.
- 화면마다 같은 계산을 반복하기보다 Store 상태를 기준으로 computed 값을 만들면 결과를 일관되게 유지할 수 있다.
- 데이터가 아직 없을 수 있으므로 온도 계산 전에 `null` 여부를 확인해야 한다.
- Pinia Store를 만들기만 해서는 사용할 수 없고 `main.js`에서 앱에 연결해야 한다.
